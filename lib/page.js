...
const loadAudio = (after) => {
  ...
  audioQueue = audioQueue
    ...
    .then(() => { if (mine === loadSeq) { audioLoaded = true; enableAudio(k === N); $("audio-note").textContent = ""; if (after) after(); } })
...
if (ABCJS.synth.supportsAudio()) {
  ...
  loadAudio();
  // Turning chords on or off rebuilds the tune for the synth, which abcjs treats as a stop. Remember where we
  // were and whether we were playing, and carry on from there, the way abcjs's own tempo control does.
  $("chords").onchange = () => {
    if (k !== N) return; // a partial score is never loaded; step N reloads it
    const wasPlaying = synthControl.isStarted, at = synthControl.percent || 0;
    loadAudio(() => {
      if (wasPlaying) synthControl.play().then(() => synthControl.seek(at)).catch(() => {});
      else if (at > 0) synthControl.runWhenReady(() => { synthControl.seek(at); synthControl.setProgress(at, synthControl.midiBuffer ? synthControl.midiBuffer.duration * 1000 : 0); return Promise.resolve(); }).catch(() => {});
    });
  };
