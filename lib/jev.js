...
// `offered` is how many options the question had. Without it a map whose foreign keys were dropped would
// look small, and a distribution spread thin over many options would read as certainty.
export function confidenceFrom(probs, offered) {
  const vals = Object.values(probs || {}).map(Number).filter((v) => Number.isFinite(v));
  if (!vals.length) return 0; // no usable distribution is no information, not certainty
  const n = Math.max(offered ?? vals.length, vals.length);
...
export function cleanProbs(probs, allowed) {
  ...
  for (const [k, v] of Object.entries(probs)) {
    if (typeof v !== "number") continue; // never coerce: null would become 0, an object could throw
    if ((!allowed || allowed.includes(k)) && Number.isFinite(v) && v >= 0 && v <= 1) out[k] = v;
  }
  return out;
}
...
export function readAnswer(a, allowed) {
  ...
  const given = typeof ok.confidence === "number" && ok.confidence >= 0 && ok.confidence <= 1 ? ok.confidence : null; // out of range is nonsense, not certainty
  const conf = usable ? given ?? confidenceFrom(probs, allowed ? allowed.length : undefined) : 0;
