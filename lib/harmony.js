...
export function describeChord(ctx, ch) {
  ...
  if (!prev) parts.push(ch.degree === 1 ? "the first chord of the piece; starting on the tonic states the key" : "the first chord of the piece; starting away from the tonic leaves the key open");
    ...
    if (m === 0) parts.push(ch.symbol === prev.symbol ? `repeats ${prev.symbol}` : `keeps the root of ${prev.symbol}, recoloured as ${ch.symbol}`);
  ...
  if (cadNext) {
    ...
    parts.push(
      ch.symbol === cadChord.symbol ? "the cadence chord itself, one bar early: the cadence would arrive as a repeat"
        : d === c ? `the cadence root one bar early, coloured as ${ch.symbol} before ${cadChord.symbol}`
        ...
        : c === 1 && d === 7 && key.mode === "major" ? `the leading-tone chord before ${cadChord.roman}: tense, resolving`
        : c === 1 && d === 7 ? `the subtonic chord before ${cadChord.roman}: a modal close`
...
export function scoreChord(ctx, ch) {
  ...
  else if (d === prev.degree) s -= 0.6; // a third bar on one root is never offered, so no further penalty is needed
...
export function buildChordState(ctx) {
  ...
  return {
    ...
    recent: {
      ...
      bars_on_the_last_root: prev ? ctx.barsOnPrev : 0,
