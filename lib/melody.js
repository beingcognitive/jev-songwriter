...
async function setup(opts, be, rand, tally, modes, hint) {
  ...
  if (questions.tempo) { tempo = TEMPOS.find((t) => t.key === read("tempo")).bpm; picks.tempo = tempo; }
...
export async function compose(opts = {}, be, onStep = () => {}) {
  ...
  // Agreement and confidence describe answers Jev actually gave; fallbacks are counted separately.
  const answered = (ts) => ts.filter((t) => t.source === "jev");
