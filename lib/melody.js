...
function makeChordCtx(S, barInfo) {
  ...
  return {
    ...
    cadChord: chordOn(barInfo.phrase.cadence === "full" ? 1 : 5, S.key),
    cadNext: barInfo.barInPhrase === barInfo.phrase.bars - 1,
  };
}
...
function describePitch(ctx, m) {
  ...
  if (!rec.lastP) parts.push(ctx.notes.length ? "the first pitch after the opening rest" : "the first note of the piece");
  else {
  ...
  if (m === range.hi) parts.push("the top of the range");
  else if (m === range.lo) parts.push("the bottom of the range");
...
function describeRest(ctx) {
  ...
  if (ctx.onset === 0 && ctx.barInfo.barInPhrase === 1) parts.push("would start the phrase with silence");
...
// How many bars in a row, just before this one, consisted of one held note (the cadence hold counts).
function heldBarsBefore(ctx) {
  let k = 0;
  for (let b = ctx.barInfo.bar - 1; b >= 1; b--) {
    const ns = ctx.notes.filter((n) => n.bar === b);
    if (ns.length === 1 && ns[0].e === EIGHTHS) k++;
    else break;
  }
  return k;
}
...
function scorePitch(ctx, m) {
  ...
  if (m === range.hi || m === range.lo) s -= 0.5;
  if (ctx.cadNext) {
...
function scoreRest(ctx) {
  ...
  if (ctx.onset === 0 && ctx.barInfo.barInPhrase === 1) s -= 1.0;
...
export function pitchOptions(ctx) {
  ...
  const restAllowed = ctx.notes.length > 0 && rec.last?.midi != null;
...
export function durationOptions(ctx) {
  ...
  const noHold = ctx.barInfo.barInPhrase === 1 || heldBarsBefore(ctx) > 0;
...
export function buildState(ctx) {
  ...
  piece.range = `${nn(range.lo)} to ${nn(range.hi)}, scale tones of ${key.name} only`;
...
async function noteStep(ctx, be, rand) {
  ...
  const pa = readAnswer(call.answers.next_pitch);
  let pick = pitches.find((o) => o.key === pa.choice), source = "jev";
  if (!pick) { pick = pitches[0]; source = "fallback"; }
  let dur = durs[0], la = null, lengthRank = null, note = durs.length === 1 ? "only one length fits the bar" : null;
  if (durs.length > 1) {
    la = readAnswer(call.answers.length);
    dur = durs.find((o) => o.key === la.choice);
    if (!dur) { dur = durs[0]; source = "fallback"; }
    lengthRank = durs.indexOf(dur) + 1;
  }
  let e = dur.e;
  if (pick.midi == null && e > MAX_REST_E) { e = MAX_REST_E; note = "rest clamped to a half note"; }
  return {
    midi: pick.midi, e, pitchKey: pick.key, lengthKey: DURATIONS.find((d) => d.e === e).key, source, note,
    answers: { next_pitch: pack(pa), length: la ? pack(la) : null },
    heuristicRank: pitches.indexOf(pick) + 1, lengthRank, optionCount: pitches.length, confidence: pa.conf, call,
  };
}
...
async function chordStep(ctx, be, rand) {
  ...
  const a = readAnswer(call.answers.chord);
...
async function setup(opts, be, rand, tally) {
  const mood = opts.mood || null;
  const questions = {};
  if (opts.mode == null && mood) questions.mode = { type: "choice", instructions: `The mode for a song whose mood is "${mood}".`, criteria: MODE_TEXT };
  if (opts.tempo == null && mood) questions.tempo = { type: "choice", instructions: `The tempo for a song whose mood is "${mood}". Each option states the pulse.`, criteria: Object.fromEntries(TEMPOS.map((t) => [t.key, t.text])) };
  if (opts.form === "auto" || (opts.form == null && mood)) {
    const at = Number(opts.tempo ?? 100);
    questions.form = { type: "choice", instructions: `The song form to write${mood ? ` for the mood "${mood}"` : ""}. Each option states its length and structure.`, criteria: Object.fromEntries(Object.keys(FORMS).map((id) => [id, describeForm(id, at)])) };
  }
  const given = { mode: opts.mode ?? "major", tempo: Number(opts.tempo ?? 100), formId: opts.form && opts.form !== "auto" ? opts.form : "aaba_32" };
  ...
  const mock = () => {
    const out = {};
    if (questions.mode) out.mode = mockChoice({ major: hint.bright ? 1.5 : hint.dark ? -0.5 : 0.5, minor: hint.dark ? 1.5 : hint.bright ? -0.5 : 0 }, rand);
    if (questions.tempo) out.tempo = mockChoice(Object.fromEntries(TEMPOS.map((t, i) => [t.key, hint.slow ? 2 - i : hint.fast ? i - 2 : -Math.abs(i - 2)])), rand);
    if (questions.form) out.form = mockChoice({ mini_8: 0.6, short_16: 1.0, aabb_16: 1.2, aaba_32: 1.4 }, rand);
    return out;
  };
  ...
  if (questions.mode) { const a = readAnswer(call.answers.mode); answers.mode = pack(a); confs.push(a.conf); mode = MODE_TEXT[a.choice] ? a.choice : "major"; picks.mode = mode; }
  if (questions.tempo) { const a = readAnswer(call.answers.tempo); answers.tempo = pack(a); confs.push(a.conf); tempo = TEMPOS.find((t) => t.key === a.choice)?.bpm ?? 100; picks.tempo = tempo; }
  if (questions.form) { const a = readAnswer(call.answers.form); answers.form = pack(a); confs.push(a.conf); formId = FORMS[a.choice] ? a.choice : "short_16"; picks.form = formId; }
...
export async function compose(opts = {}, be, onStep = () => {}) {
  ...
  const mood = preset ? preset.text : opts.mood || null;
  const sevenths = opts.sevenths != null ? !!opts.sevenths : !!preset?.sevenths;
  const { mode, tempo, formId, setupCall } = await setup({ ...opts, mood }, be, rand, tally);
  if (!(tempo >= 40 && tempo <= 240)) throw new Error(`tempo must be 40..240 bpm, got ${opts.tempo}`);
  const key = keyInfo(opts.key ?? "C", mode);
  const range = { lo: key.tonicMidi - RANGE.below, hi: key.tonicMidi + RANGE.above };
  ...
  const notes = [], trace = [];
  const S = { key, tempo, formId, form, planBars, total, range, notes, mood, sevenths };
    ...
    if (ph.reuse) {
      ...
      for (const b of phBars.slice(0, -1)) {
        ...
        while (onset < EIGHTHS) {
          ...
          record({
            kind: "jev", bar: b.bar, beat: beatStr(onset), onset, pitch: r.pitchKey, length: r.lengthKey, eighths: r.e, note: r.note, source: r.source,
  ...
  const title = opts.title ?? (moodLabel ? `${moodLabel[0].toUpperCase()}${moodLabel.slice(1)}` : `Jev in ${key.name}`);
  return {
    ...
    range: `${noteName(range.lo, key.flats)}–${noteName(range.hi, key.flats)}`,
    ...
    stats: {
      ...
      agreement: jev.length ? jev.filter((t) => t.heuristicRank === 1).length / jev.length : null,
      chordAgreement: chords.length ? chords.filter((t) => t.heuristicRank === 1).length / chords.length : null,
      meanConfidence: jev.length ? jev.reduce((a, t) => a + t.confidence, 0) / jev.length : null,
    },
  };
}
