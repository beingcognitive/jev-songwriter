...
// An ABC header field ends at the newline, so a title with one in it would inject fields of its own.
const field = (t, fallback) => String(t ?? "").replace(/[\r\n]+/g, " ").trim() || fallback;

export function toAbc(result) {
  const { notes, barPlan, flats } = result;
  const composer = result.model === "jev-latest" ? "Jev (TypeSafe System One)" : `Jev stand-in (${field(result.model, "mock")})`;
  let s = `X:1\nT:${field(result.title, "Untitled")}\nC:${composer}\nM:4/4\nL:1/8\nQ:1/4=${result.tempo}\nK:${result.abcKey}\n`;
  const spans = [];
  for (const info of barPlan) {
    ...
    ns.forEach((n, i) => {
      ...
      prevHalf = half;
      if (i === 0) s += `"${info.chord}"`;
      const start = s.length; // the span is the note token itself; the chord symbol before it belongs to the bar
